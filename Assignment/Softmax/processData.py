# -*- coding: utf-8 -*-
"""
Created on Sat Apr 21 21:27:18 2018

@author: GFX
"""

# 1、使用向量化方法计算softmax损失函数
# 2、使用向量化方法分析梯度
# 3、使用数值梯度法与分析梯度法比较来检查梯度
# 4、用验证集来微调学习率和正则项
# 5、使用随机梯度下降法来优化损失函数
# 6、可视化权重
...
print('loss(数值计算 参数随机生成) is : %f \n' % loss)  # 因为随机产生参数，每次运行结果会略有变化
# 梯度检验
# 将数值梯度法与分析梯度法计算结果进行比较
print('将数值梯度法与分析梯度法计算结果进行比较:\n')
print('数值梯度法结果：')
...
# 两种方式比较
print('数值梯度法与分析梯度法在结果与时间上比较：')
tic = time.time()
loss_naive, grad_naive = softmax_loss_naive(w, x_dev, y_dev, 0.00001)
toc = time.time()
print('\n naive loss: %e computed in %f s' % (loss_naive, toc - tic))#貌似结果不对
tic = time.time()
...
x_scatter = [math.log10(x[0]) for x in results]  # 学习率
y_scatter = [math.log10(x[1]) for x in results]  # 正则项

sz = [results[x][0] * 1500 for x in results]  # 使用面积表示正确率大小（几乎看不出差别）
plt.subplot(1, 2, 1)
...
# 6 权重可视化
w = best_softmax.W[:-1, :]  # 将偏置b分离出来，只可视化权重
